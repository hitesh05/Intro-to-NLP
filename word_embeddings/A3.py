...
class cbow_word2vec(nn.Module):
    def __init__(self, vocab_size, embed_size, pad_val):
        ...
        self.in_embeddings.weight.data.uniform_(-1.0, 1.0)
...
def find_nearest_words(word_embeddings, word_to_idx, idx_to_word, word, choice=1, num=10):
    try:
...
def plot_tsne(embeddings, words, word_to_idx, idx_to_word, num=10):
    similar_word_indices = list()
    for word in words:
        try:
            idx = word_to_idx[word]
        except:
            idx = word_to_idx['<UNK>']
        similar_word_indices.append(find_nearest_words(embeddings, word_to_idx, idx_to_word, word, choice=0))
...
if __name__ == '__main__':   
    ...
    plot_tsne(word_embeddings_svd, words, word_to_idx, idx_to_word)
    plot_tsne(word_embeddings_w2v, words, word_to_idx, idx_to_word)
